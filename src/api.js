...
const getToken = async (code) => {
  const encodeCode = encodeURIComponent(code);
  const { access_token } = await fetch(
    `https://raro370h4k.execute-api.eu-central-1.amazonaws.com/dev/api/token/${encodeCode}`
  )
    .then((res) => {
      return res.json();
    })
...
export const getEvents = async () => {
  ...
  if (token) {
    removeQuery();
    const url = `https://raro370h4k.execute-api.eu-central-1.amazonaws.com/dev/api/get-events/${token}`;
